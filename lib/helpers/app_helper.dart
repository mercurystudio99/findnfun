...
class AppHelper {
  ...
  // Remove all the pending purchases
  Future<void> removePendingPurchases() async {
    // Check platform
    if (Platform.isIOS) {
      ...
      if (transactions.isNotEmpty) {
        ...
        for (var transaction in transactions) {
          await paymentWrapper
              ...
              .catchError((onError) {
            debugPrint('removePendingPurchases() -> error: $onError');
          });
        }
        // Debug
        debugPrint('removePendingPurchases() -> completed');
      } else {
        // Debug
        debugPrint('removePendingPurchases() -> no pending transactions');
      }
    }
  }

  DateTime? getTransactionDate(PurchaseDetails purchase) {
    // Get transaction date
    String? transactionDate = purchase.transactionDate;

    // Parse transactionDate String to int
    final int? millisecondsSinceEpoch = int.tryParse("$transactionDate");
