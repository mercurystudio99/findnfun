...
class UserModel extends Model {
  ...
  Future<void> signUp({
    ...
    await _firestore
      ...
      _fcm.subscribeToTopic(NOTIFY_USERS);
    }).catchError((onError) {
      isLoading = false;
      notifyListeners();
  ...
  int getFreeActivities() {
    int maxActivities = 0;

    if (!UserModel().userIsVip) {
      maxActivities = AppModel().appInfo.freeAccountMaxActivities;
      debugPrint('checkUserMaxDistance() -> User is not VIP Account.' +
          maxActivities.toString());
    }
