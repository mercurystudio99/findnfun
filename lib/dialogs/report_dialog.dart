...
import 'package:dating_app/plugins/stories/widgets/flag_story_dialog.dart';
...
class ReportDialog extends StatefulWidget {
  // Constructor
  const ReportDialog(
      {Key? key, this.userId, this.story, this.isStoryProfile = false})
      : super(key: key);
  ...
  void show() {
    ...
    final _modal = ReportDialog(
        userId: userId, story: story, isStoryProfile: isStoryProfile);
    ...
    if (Platform.isIOS) {
      ...
      // Android modal
      showModalBottomSheet(context: _context, builder: (context) => _modal);
    }
  }
    ...
    showDialog(
        ...
        builder: (context) {
          if (widget.story != null) {
            return FlagStoryDialog(
                story: widget.story!, isStoryProfile: widget.isStoryProfile);
          }
          return FlagUserDialog(flaggedUserId: widget.userId!);
        });
  }
